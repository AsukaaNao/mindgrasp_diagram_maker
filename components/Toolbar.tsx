...
export const Toolbar: React.FC<ToolbarProps> = ({ fileManager, diagramState, diagramController, onShowToast, isSaving }) => {
  ...
  const handleSave = async () => {
      ...
      try {
          await saveToBrowser();
          onShowToast('Diagram saved', 'success');
      } catch (err) {
      ...
      <div className="flex gap-2 items-center">
        ...
        <button
            ...
            className={`px-3 py-1.5 rounded text-xs font-semibold transition flex items-center gap-2 mr-2 uppercase tracking-wide
                ${hasSelection 
                    ? 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-100 border border-red-200 dark:border-red-900' 
                    : 'text-gray-300 dark:text-gray-700 border border-transparent cursor-not-allowed'}`}
        >
            Delete
        </button>
        ...
        <button
          onClick={handleSave}
          disabled={isSaving}
          className={`px-4 py-2 bg-black dark:bg-blue-600 text-white hover:bg-gray-800 dark:hover:bg-blue-500 rounded-lg text-sm font-medium transition shadow-sm flex items-center gap-2
        ...
        <div className="flex gap-1">
            <button onClick={handleDownloadJson} className="px-3 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-lg text-xs font-medium transition">JSON</button>
            <button onClick={handleExportSvg} className="px-3 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-lg text-xs font-medium transition">SVG</button>
        </div>
      </div>
    </div>
  );
